...
describe('Gaudome netinkamus tipus', () => {      ///testu grupiu funkcija, ieskome ar atitinka ivestas tipas reikiama tipa.

    test('no params', () => {                        //nesuvesti duomenys
        const [err, msg] = IsValid.fullname();       //destrukturizuoju jau zinomas reiksmes is IsValid, priskiriu jiems parametrus err, msg, pritaikau reikiama IsValid klases metoda (fullname siuo atveju.)
        expect(err).toBe(true);                     //tikriname ar eroras yra true reiksme, (ar yra eroras). Tikimes gauti true, kad klaida yra.
        expect(msg).toBe('Neduotas parametras');    //tikriname ar zinute yra ta.
    })
...
describe('Gauname netinkamas reiksmes', () => {
    ...
    test('single word', () => {
        ...
        expect(err).toBe(true);
        expect(msg).toBe('Tekstas turi tureti 2 arba daugiau zodziu');
    })
...
describe('Email', () => {
    ...
    describe('Gauname tinkamas reiksmes', () => {
        ...
        test('ok (2)', () => {
            const [err, msg] = IsValid.email('petras@inbox.petras.xyz');
            expect(err).toBe(false);
            expect(msg).toBe('OK');
        })
