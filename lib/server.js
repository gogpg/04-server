...
server.httpServer = http.createServer(async (req, res) => {  //funkcija, kuri perima internetini srauta ir iskviecia musu paciu pasirinkta serverio veikimo logika.
   ...
   /* const cachePeriod = {
    ...
    if (isPage) {
      ...
      //1
        /*let pageFunction = server.routes[404];   //vienas variantas
        if (trimmedPath in server.routes) {
            pageFunction = server.routes[trimmedPath];
        }
        responseContent = pageFunction();*/
    //2
        responseContent = server.routes[trimmedPath] ? server.routes[trimmedPath]() : server.routes[404]() // kitas variantas
    }
